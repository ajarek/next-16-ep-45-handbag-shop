...
interface AuthContextType {
  ...
  zaktualizujProfil: (uidOrDane: string | Partial<UserProfile>, maybeDane?: Partial<UserProfile>) => Promise<void>;
  zalogowany: boolean;
  isAdmin: boolean;
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const utworzProfil = useCallback(
    async (user: User, nazwa: string) => {
  ...
  const zaktualizujProfil = useCallback(
    async (uidOrDane: string | Partial<UserProfile>, maybeDane?: Partial<UserProfile>) => {
  ...
  return (
    <AuthContext.Provider
      value={{
        ...
        isAdmin: !!uzytkownik && uzytkownik.email === (process.env.NEXT_PUBLIC_ADMIN_EMAIL || ""),
      }}
    >
      {children}
    </AuthContext.Provider>
