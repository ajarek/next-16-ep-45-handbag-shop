...
export function AdminFinance() {
  ...
  const [aktywnyWykres, setAktywnyWykres] = useState<"tydzien" | "miesiac">("tydzien");
  ...
  const dane = useMemo(() => {
    ...
    const przychodPM = wszystkie
      .filter((z) => {
        const d = new Date(z.dataZlozenia);
        return d.getMonth() === poprzedniMiesiac && d.getFullYear() === poprzedniRok;
      })
      .reduce((sum, z) => sum + z.razem, 0);
    ...
    // ─── Top produkty ───
    const produktMap = new Map<string, { nazwa: string; ilosc: number; przychod: number }>();
    ...
    const metodyPlatnosci: MetodaPlatnosci[] = Array.from(platnosciMap.entries())
      .map(([klucz, wart]) => ({
        nazwa:
          klucz === "blik" ? "BLIK" :
          klucz === "karta" ? "Karta" :
          klucz === "payu" ? "PayU" : "Apple Pay",
    ...
    return {
      przychodLaczny, przychodDostarczone, srednia,
      przychodBM, zmianaProc, zamowieniaBM,
      tydzien, miesiace, topProdukty, metodyPlatnosci,
  ...
  if (blad) {
    return (
      <div className="p-4 rounded-2xl bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800/50 text-sm text-rose-600 dark:text-rose-400 font-medium text-center">
        {blad}
        <button onClick={pobierzDane} className="ml-3 underline hover:no-underline">
          Spróbuj ponownie
        </button>
      </div>
    );
      ...
      <div className="p-6 rounded-2xl bg-card border border-border space-y-6">
        ...
        {aktywnyWykres === "tydzien" ? (
          <ChartContainer config={wykresConfig} className="h-70 w-full">
            <BarChart data={dane.tydzien} margin={{ top: 20, right: 10, left: 10, bottom: 0 }}>
              <XAxis
                dataKey="nazwa"
                tickLine={false}
                axisLine={false}
              ...
              <ChartTooltip
                content={
                  <ChartTooltipContent
                    formatter={(value) => [formatPrice(Number(value)), "Przychód"]}
          ...
          <ChartContainer config={wykresConfig} className="h-70 w-full">
            <BarChart data={dane.miesiace} margin={{ top: 20, right: 10, left: 10, bottom: 0 }}>
              <XAxis
                dataKey="nazwa"
                tickLine={false}
                axisLine={false}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-2xl bg-card border border-border space-y-4">
          ...
          {dane.topProdukty.length === 0 ? (
            <p className="text-xs text-muted-foreground text-center py-6">Brak danych o sprzedaży.</p>
          ) : (
                  ...
                  <div key={idx} className="space-y-1.5">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="text-xs font-medium text-foreground truncate">{produkt.nazwa}</span>
                      </div>
                      <div className="flex items-center gap-3 shrink-0">
                        <span className="text-[10px] text-muted-foreground">{produkt.ilosc} szt.</span>
                        <span className="text-xs font-semibold text-foreground">{formatPrice(produkt.przychod)}</span>
                      </div>
                    </div>
        ...
        <div className="p-6 rounded-2xl bg-card border border-border space-y-4">
          ...
          {dane.metodyPlatnosci.length === 0 ? (
            <p className="text-xs text-muted-foreground text-center py-6">Brak danych o płatnościach.</p>
          ) : (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {dane.metodyPlatnosci.map((metoda, idx) => {
                  ...
                  const calkowityPrzychod = dane.metodyPlatnosci.reduce((sum, m) => sum + m.przychod, 0);
                  const procent = calkowityPrzychod > 0 ? (metoda.przychod / calkowityPrzychod) * 100 : 0;
                  ...
                  return (
                    <div key={idx} className="flex items-center gap-3 p-3 rounded-xl bg-secondary/50 border border-border/50">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-medium text-foreground">{metoda.nazwa}</span>
                          <span className="text-xs font-semibold text-foreground">{formatPrice(metoda.przychod)}</span>
                        </div>
                        <div className="flex items-center gap-2 mt-1">
                          <div className="flex-1 h-1 rounded-full bg-background overflow-hidden">
                            <div className="h-full rounded-full bg-accent/60" style={{ width: `${procent}%` }} />
                          </div>
                          <span className="text-[10px] text-muted-foreground">{metoda.liczba} · {procent.toFixed(0)}%</span>
                        </div>
                      </div>
                    </div>
                  );
...
function KpiCard({
  tytul, wartosc, opis, ikona: Icon, kolor, bgKolor, zmiana,
}: {
  tytul: string;
  wartosc: string;
  opis: string;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <div className={`w-9 h-9 rounded-xl ${bgKolor} flex items-center justify-center`}>
        ...
        {zmiana !== undefined && zmiana !== 0 && (
          ...
          >
            {zmiana > 0 ? <ArrowUpRight className="w-3.5 h-3.5" /> : <ArrowDownRight className="w-3.5 h-3.5" />}
            {Math.abs(zmiana).toFixed(1)}%
          </div>
        )}
      </div>
      <div>
        <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold mb-1">{tytul}</p>
        <p className="font-serif text-xl font-bold text-foreground">{wartosc}</p>
...
function WykresKolowy({
  ...
  metody: { nazwa: string; przychod: number; ikona: React.ComponentType<{ className?: string }> }[];
}) {
  ...
  if (calkowity === 0) {
    return (
      <div className="rounded-full border-4 border-dashed border-border" style={{ width: rozmiar, height: rozmiar }} />
    );
  }
  ...
  return (
    <svg width={rozmiar} height={rozmiar} viewBox={`0 0 ${rozmiar} ${rozmiar}`}>
      {sciezki}
      <circle cx={srodek} cy={srodek} r={promien * 0.55} className="fill-card" />
      <text x={srodek} y={srodek - 4} textAnchor="middle" className="fill-foreground" fontSize="8" fontWeight="700">
        {calkowity >= 1000 ? `${(calkowity / 1000).toFixed(1)}k` : calkowity.toFixed(0)}
      </text>
      <text x={srodek} y={srodek + 8} textAnchor="middle" className="fill-muted-foreground" fontSize="4">
        zł łącznie
      </text>
    </svg>
  );
