...
import { X, Eye, EyeOff, Mail, Lock, ArrowRight, LogIn } from "lucide-react"
...
export function CheckoutAuthModal({
  ...
  const { logowanie, logowanieGoogle } = useAuth()
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (kod === "auth/popup-closed-by-user") {
        ...
        setBlad("Wystąpił błąd logowania przez Google. Spróbuj ponownie.")
      }
    } finally {
      ...
      {otwarty && (
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
          ...
          <motion.div
            ...
            className='relative w-full max-w-md bg-card text-card-foreground rounded-2xl shadow-2xl border border-border overflow-hidden z-10'
          >
            {/* Przycisk zamknięcia */}
            <button
              onClick={handleClose}
            ...
            <div className='p-6 pb-0 text-center space-y-3'>
              ...
              <p className='text-xs text-muted-foreground font-light leading-relaxed'>
                Twoje dane formularza zostały zachowane.
                <br />
                Zaloguj się, aby dokończyć zamówienie.
              </p>
            </div>
            ...
            <div className='px-6 pt-4 pb-2'>
              <button
                ...
                className='w-full flex items-center justify-center gap-3 px-6 py-3 rounded-xl bg-background border border-border text-xs font-semibold text-foreground hover:bg-secondary transition-colors disabled:opacity-50'
              >
                {ladujeGoogle ? (
                  <>
                    <div className='w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin' />
            ...
            {/* Akcent wizualny — delikatna ikonaSparkles na dole */}
            <div className='absolute bottom-0 left-0 right-0 h-1 bg-linear-to-r from-accent/0 via-accent/40 to-accent/0' />
          </motion.div>
        </div>
      )}
    </AnimatePresence>
