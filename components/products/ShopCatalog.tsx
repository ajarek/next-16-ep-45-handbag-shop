...
export function ShopCatalog({ activeCategory }: ShopCatalogProps) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    const matchesTag = (product: Product) =>
      selectedTag === "all" ||
      product.tags.some((tag) => tag.toLowerCase().includes(selectedTag.toLowerCase()));

    const filtered = products.filter((product) => matchesCategory(product) && matchesTag(product));
    ...
    <section className="py-16 bg-background text-foreground transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-4 mb-4 pb-6 border-b border-border">
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
            {categories.map((category) => (
              <Link
                key={category.id}
                href={category.id === "all" ? "/shop" : `/shop?kategoria=${category.id}`}
                scroll={false}
        ...
        <p className="mb-8 text-xs uppercase tracking-[0.2em] font-semibold text-muted-foreground">
          Znaleziono:{" "}
          <span className="text-accent">{formatModelsCount(filteredProducts.length)}</span>
        </p>
        ...
        <AnimatePresence mode="popLayout">
          {filteredProducts.length > 0 ? (
            ...
            <div className="text-center py-20 flex flex-col items-center gap-3">
              ...
              <p className="text-base text-muted-foreground">Brak modeli spełniających wybrane kryteria.</p>
              <Link
                href="/shop"
