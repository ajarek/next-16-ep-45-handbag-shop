...
export async function zapiszZamowienie(
...
export async function pobierzWszystkieZamowienia(): Promise<
  ZamowienieFirestore[]
> {
  const q = query(collection(db, "zamowienia"), orderBy("data", "desc"))
  const snapshot = await getDocs(q)
...
export async function dodajRecenzje(
  recenzja: Omit<RecenzjaFirestore, "id">,
): Promise<string> {
  ...
  if (produktSnap.exists()) {
    ...
    const nowaOcena = (obecnaOcena * obecnaLiczba + recenzja.ocena) / nowaLiczba
...
export async function pobierzKoszyk(uzytkownikId: string): Promise<CartItem[]> {
  const docRef = doc(db, "koszyki", uzytkownikId)
  const docSnap = await getDoc(docRef)
  if (!docSnap.exists()) return []
